                                                        ...
                                                        <% if (style.includeCategorySearch) {%>
	                                                        ...
	                                                        if(CategoryFactory.isValidXmlCategoryKey(conn, catKey)){
		                                                        for(int i=origin.length();i<=catKey.length();i+=Constants.CHARS_PER_CATEGORY){%>
																	<a style="text-decoration: none; background-color: lime; padding: 2px;" href="<%= catgenContext.RelativePath%>Search.html?category=<%=catKey.substring(0,i)%>&pagesize=<%=catgenContext.PageSize%>&singlePageTemplate=true&test=correct&pricerange=1">
																	<%=CategoryFactory.getCategoryNameByCategoryKeyFromCategory(conn,catKey.substring(0,i),networkMarket.NetworkMarketID) %></a>&nbsp;&gt;&nbsp; 
																<%} 
															}%>
