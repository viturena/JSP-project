<%-- With Referrals Geolocation Changes: Feb 2010  --%>
<%@ page import="java.util.*, org.apache.commons.lang.*,java.sql.*,com.catgen.*,com.catgen.factories.*,java.net.URLEncoder, java.text.*, com.catgen.Constants" %><%
	...
	try {
		...
		{
			String m = request.getParameter("marketid");
			if(m != null && m.length() > 0)
			{
				referrals = ReferralFactory.getReferralInfoByMarketID(conn, m, startDate, endDate2);
			}
		}
...
<title>OpenEntry Referral Reports - from Referrers to Network Markets</title>
<style>
body {
	margin-left: 0px;
	margin-top: 0px;
...
<body>
<center><h3>OpenEntry Referral Reports - from Referrers to Network Markets  ( <a href="ReferralsCompany.jsp">Companies</a> | <a href="Referrals.jsp">Referrers</a> ) </h3></center>

<form action="ReferralsMarket.jsp" method="post">
<table width=70% align=center>
<tr>
<td>
<input type="hidden" name="action" value="1"/>
Market code:  
<input type="text" name="marketid" value="<%= StringEscapeUtils.escapeHtml( Utils.getSafeString( request.getParameter("marketid" ))) %>"/> 
</td>
...
<script type="text/javascript">
	...
	<%
		...
		{    
	%>
	<center>Check where in the world people are browsing from by copying the Client IP into <a href="http://WhatIsMyIPAddress.com" target="_blank">http://WhatIsMyIPAddress.com</a><br/><br /><b><%= referrals.size() %></b> Referrals for <b>"<%= request.getParameter("marketid" ) %>"</b> between <b><%= searchDateFormat.format(startDate) %></b> and <b><%= searchDateFormat.format(endDate) %></b> 
	<a href="ReferralsMarket.jsp?marketid=<%= request.getParameter("marketid" ) %>&startdate=<%= searchDateFormat.format(startDate) %>&enddate=<%= searchDateFormat.format(endDate) %>">Download as Tab Delimited Excel File</a></center><br />
	...
	{
	%>
		<span style="font-size: x-large">No referral events found</span>
	<%
	}
	%>
