...
<title>OpenEntry Referral Reports</title>
<style>
body {
	margin-left: 0px;
	margin-top: 0px;
...
<body>
<center><h3>OpenEntry Referral Reports  ( <a href="ReferralsMarket.jsp">Network Markets</a> | <a href="ReferralsCompany.jsp">Companies</a> ) </h3></center>

<form action="Referrals.jsp" method="post">
<table width=70% align=center>
<tr>
<td>
<input type="hidden" name="action" value="1"/>
Referral ID:  
<input type="text" name="referralid" value="<%= StringEscapeUtils.escapeHtml( Utils.getSafeString( request.getParameter("referralid" ))) %>"/> 
</td>
...
<script type="text/javascript">
    ...
    %>
	<center>
	Check where in the world people are browsing from by copying the Client IP into <a href="http://WhatIsMyIPAddress.com" target="_blank">http://WhatIsMyIPAddress.com</a><br/><br /><b><%= referrals.size() %></b> Referrals for <b>"<%= request.getParameter("referralid" ) %>"</b> between <b><%= searchDateFormat.format(startDate) %></b> and <b><%= searchDateFormat.format(endDate) %></b> 
	<a href="Referrals_txt.jsp?referralid=<%= request.getParameter("referralid" ) %>&startdate=<%= searchDateFormat.format(startDate) %>&enddate=<%= searchDateFormat.format(endDate) %>">Download as Tab Delimited Excel File</a></center><br />
